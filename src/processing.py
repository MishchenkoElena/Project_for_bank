...
def sort_by_date(list_of_dictionaries: list, sort_order: bool = True) -> list:
    '''
    Функция принимает список словарей и необязательный параметр, задающий порядок сортировки (по умолчанию — убывание) и возвращает новый список, отсортированный по дате (date)
    '''
    sorted_list_of_dictionaries = sorted(list_of_dictionaries, key=lambda dictionary: dictionary ['date'], reverse=sort_order)
